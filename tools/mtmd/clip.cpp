...
// ------------------------------------------------------------------------
    // Helper to fix 1x1 conv weight shapes
    // ------------------------------------------------------------------------
    ggml_tensor* fix_1x1_weight(ggml_tensor* w) {
        if (!w) return nullptr;
        // GGUF loaded 1x1 weights as [Cout, Cin, 1, 1] (ne indices)
        // ggml_conv_2d expects [1, 1, Cin, Cout]
        // Permute dimensions: 0->3, 1->2, 2->0, 3->1
        ggml_tensor* w_p = ggml_permute(ctx0, w, 2, 3, 1, 0);
        return ggml_cont(ctx0, w_p);
    }
    ...
    ggml_tensor * build_mobilenet_attn(ggml_tensor * inp, const mobilenetv5_block & block, int block_idx = -1) {
        // Debug input to blocks 50 and 52
        ...
        if ((block_idx == 50 || block_idx == 52) && block.attn_norm_w) {
            char debug_name[128];
        ...
        // Debug Q after conv for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug K after conv for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug V after conv for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug intermediate tensors for blocks 50 and 52 (for comparison)
        if (block_idx == 50 || block_idx == 52) {
        ...
        // Debug k_broadcast for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug scores before scaling for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug scores after scaling for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug v_broadcast for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug kqv before projection for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        // Debug after output projection for blocks 50 and 52
        if (block_idx == 50 || block_idx == 52) {
            char debug_name[128];
        ...
        if (inp->ne[0] == cur->ne[0] && inp->ne[2] == cur->ne[2]) {
            if (block.layer_scale_w) {
                ...
                // Debug after layer_scale for blocks 50 and 52
                if (block_idx == 50 || block_idx == 52) {
                    char debug_name[128];
            ...
            // Debug final output for blocks 50 and 52
            if (block_idx == 50 || block_idx == 52) {
                char debug_name[128];
    ...
    // ------------------------------------------------------------------------
    // MobileNetV5 Builder (Gemma 3n) - FIXED
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    ...
    ggml_cgraph * build_mobilenetv5() {
        ...
        for (int i = 0; i < total_blocks; i++) {
            ...
            if (is_fusion_point(i)) {
                ...
                DEBUG_SHAPE("Fusion Feature", cur);
...
struct clip_model_loader {
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        if (model.proj_type == PROJECTOR_TYPE_GEMMA3N) {
            ...
            get_tensor(TN_MNV5_MSFA_FFN_EXP_BN, false); // Consume BN if present but likely folded
            ...
            for (int stage = 0; stage < 4; ++stage) {
                ...
                for (int blk_idx = 0; ; ++blk_idx) {
                    ...
                    // 3. Check for Attention (MQA) - THIS WAS THE MISSING LINK
